...
docker exec openclaw openclaw chat --message "Xin chào, con là ai?" --once
echo ""

# Test 2: Vietnamese kinship terms
echo "Test 2: Kinship terms (should use 'con' and 'bà')"
docker exec openclaw openclaw chat --message "Giúp bà đọc email" --once
echo ""

# Test 3: Form filling instructions
echo "Test 3: Form filling in Vietnamese"
docker exec openclaw openclaw chat --message "Bà muốn điền biểu mẫu trên trang dichvucong.gov.vn" --once
echo ""

# Test 4: Error handling in Vietnamese
echo "Test 4: Error handling"
docker exec openclaw openclaw chat --message "Trang web bị lỗi, làm sao bây giờ?" --once
echo ""
...
echo "  Then: docker compose restart openclaw"
