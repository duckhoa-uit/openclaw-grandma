...
# Access via browser at http://your-vps-ip:8080
set -euo pipefail
...
case "$ACTION" in
  start)
    ...
    echo "WARNING: noVNC exposes a desktop on port 8080. Stop it when done!"
    echo ""
    docker compose --profile setup up -d novnc
    echo ""
    echo "noVNC is running. Open http://YOUR-VPS-IP:8080 in your browser."
